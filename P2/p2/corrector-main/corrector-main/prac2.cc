...
// JAVIER VILLANUEVA MARTÍNEZ   51299727K
...
bool comprobadorDeRespuesta(string currentLine, int linea) {
    for(unsigned int i = 0; i < currentLine.size(); i++) {
        if(currentLine[i]=='|' && i + 1 >= currentLine.size()) {    // El programa recorre la línea buscando una barra en posición final
                                                                    // cosa que indicaría que el campo respuesta está vacío
            cout << "Error line " << linea << endl;
            return true;
        }
    }
...
bool comprobadorValidezNombreProfesor(string &nombreProfesor) {
    ...
    if(nombreProfesor[0]==' ')nombreProfesor.erase(nombreProfesor.begin());
    if(nombreProfesor[nombreProfesor.size() - 1]==' ')nombreProfesor.erase(nombreProfesor.begin() + nombreProfesor.size() - 1);
